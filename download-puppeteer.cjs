...
(async () => {
  console.log("Launching browser (non-headless to avoid bot detection)...");
  ...
  // Download images one at a time with long delays
  let ok = 0, fail = 0;

  for (let i = 0; i < allImagePaths.length; i++) {
    const imgPath = allImagePaths[i];
    const dest = path.join(OUT, imgPath);

    // Skip if already valid
    if (fs.existsSync(dest)) {
      const buf = fs.readFileSync(dest);
      const head = buf.slice(0, 4).toString('hex');
      // JPEG starts with ffd8, PNG with 89504e47, SVG with 3c737667 or 3c3f786d
      if (head.startsWith('ffd8') || head.startsWith('8950') || head.startsWith('3c73') || head.startsWith('3c3f') || head.startsWith('3c21')) {
        // Wait, 3c21 is <!DOCTYPE which is HTML - skip that
        if (!head.startsWith('3c21')) {
          console.log(`SKIP (valid): ${imgPath}`);
          ok++;
          continue;
        }
      }
    ...
    const url = `https://airbnb-clone-umber-two.vercel.app/${imgPath}`;
    ...
    try {
      ...
      if (response && response.ok()) {
        ...
        if (head.startsWith('ffd8') || head.startsWith('8950') || head.startsWith('3c73') || head.startsWith('3c3f')) {
          fs.mkdirSync(path.dirname(dest), { recursive: true });
          fs.writeFileSync(dest, buffer);
          console.log(`[${i+1}/${allImagePaths.length}] Saved: ${imgPath} (${buffer.length} bytes)`);
          ok++;
        } else if (imgPath.endsWith('.svg')) {
          // SVGs might start differently
          fs.mkdirSync(path.dirname(dest), { recursive: true });
          fs.writeFileSync(dest, buffer);
          console.log(`[${i+1}/${allImagePaths.length}] Saved SVG: ${imgPath} (${buffer.length} bytes)`);
          ok++;
        } else {
          console.log(`[${i+1}/${allImagePaths.length}] BAD: ${imgPath} (header: ${head})`);
          fail++;
        }
      } else {
        console.log(`[${i+1}/${allImagePaths.length}] HTTP ${response?.status()}: ${imgPath}`);
        fail++;
      ...
      // Delay between requests (1-2s random)
      const delay = 1000 + Math.random() * 1000;
      await new Promise(r => setTimeout(r, delay));
    } catch (e) {
      console.log(`[${i+1}/${allImagePaths.length}] ERROR: ${imgPath} - ${e.message}`);
      fail++;
