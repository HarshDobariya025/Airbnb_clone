...
const CalendarIcon = () => (
  <svg viewBox="0 0 32 32" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" style={{ display: 'block', height: '100%', width: '100%', fill: 'currentColor' }}>
...
export default function ThingsToKnow() {
  return (
    <section className="ttk">
      <h2 className="ttk__title">Things to know</h2>
      <div className="ttk__grid">
        {sections.map(s => (
          <div className="ttk__col" key={s.title}>
            <div className="ttk__col-icon"><s.Icon /></div>
            <div className="ttk__col-title">{s.title}</div>
            {s.items.map((item, i) => <p className="ttk__item" key={i}>{item}</p>)}
            <a className="ttk__learn" href="#">Learn more</a>
          </div>
        ))}
      </div>
    </section>
